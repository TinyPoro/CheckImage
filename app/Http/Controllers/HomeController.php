...
class HomeController extends Controller
{
    ...
    public function reportSummary(Request $request)
    {
        ...
        // from (SELECT src, concat(src,concat(chuong_trinh,concat(khu_vuc,concat(do_kho,ten_nguon)))) as info from check_image) tmp
        // GROUP BY info) tbl1
        // join (select src, count(*) as cnt
        // from check_image
        // GROUP BY src) tbl2 on (tbl1.info like concat('%',concat(tbl2.src,'%')))
        // )
        // where 10*tbl1.cnt >= 6*tbl2.cnt
        $infos = array_map(function($item){
            return $item->info;
        },\DB::select(\DB::raw("SELECT tbl1.info from ((SELECT info, count(*) as cnt from (SELECT src, concat(src,concat(chuong_trinh,concat(khu_vuc,concat(do_kho,ten_nguon)))) as info from check_image) tmp GROUP BY info) tbl1 join (select src, count(*) as cnt from check_image GROUP BY src) tbl2 on (tbl1.info like concat('%',concat(tbl2.src,'%')))) where 10*tbl1.cnt >= 6*tbl2.cnt")));
        // dd($infos);
        foreach ($curriculums as $cur_key => $curriculum) {
            foreach ($kinds as $kind)
                $values[$cur_key . $kind] = 0;
            foreach ($curriculum as $cha_key => $chapter) {
                ...
                foreach ($chapter as $lesson) {
                    foreach ($kinds as $kind) {
                        $values[$cur_key . $cha_key . $lesson . $kind] = \DB::table('check_image')->where('chuong_trinh', $lesson)->whereRaw("concat(khu_vuc,concat(do_kho,ten_nguon))='" . $kind . "'")->whereIn(\DB::raw('concat(src,concat(chuong_trinh,concat(khu_vuc,concat(do_kho,ten_nguon))))'), $infos)->distinct('src')->count('src');
                        // \DB::select(\DB::raw("SELECT count(src) as count from ( select src from check_image " .
                        //     "where chuong_trinh = '" . $lesson . "' and concat(khu_vuc,concat(do_kho,ten_nguon))='" . $kind . "' GROUP BY src having count(*)>=3) as tbl"))[0]->count;
                        $values[$cur_key . $cha_key . $kind] += $values[$cur_key . $cha_key . $lesson . $kind];
                    }
                }
                foreach ($kinds as $kind)
        ...
        foreach ($kinds as $kind)
            $values['Không xác định' . $kind] = \DB::table('check_image')->where('chuong_trinh', 'Không xác định')->whereRaw("concat(khu_vuc,concat(do_kho,ten_nguon))='" . $kind . "'")->whereIn(\DB::raw('concat(src,concat(chuong_trinh,concat(khu_vuc,concat(do_kho,ten_nguon))))'), $infos)->distinct('src')->count('src');
            // $values['Không xác định' . $kind] = \DB::select(\DB::raw("SELECT count(src) as count from ( select src from check_image " .
            // "where chuong_trinh = '" . 'Không xác định' . "' and concat(khu_vuc,concat(do_kho,ten_nguon))='" . $kind . "' GROUP BY src having count(*)>=3) as tbl"))[0]->count;
        ...
        return view('report-summary', ['curriculums' => $curriculums, "values" => $values, "kinds" => $kinds]);
    }
}
